...
JamJar::Standard::WebGL2System::WebGL2System(MessageBus *messageBus, EMSCRIPTEN_WEBGL_CONTEXT_HANDLE context)
    : RenderSystem(messageBus, WebGL2System::evaluator), m_context(context) {}

// NOLINTNEXTLINE(misc-unused-parameters)
bool JamJar::Standard::WebGL2System::evaluator(Entity *entity, std::vector<JamJar::Component *> components) {
    bool hasCamera = false;
    bool hasTransform = false;
    for (const auto &component : components) {
        if (component->m_key == JamJar::Standard::Camera::CAMERA_KEY) {
            hasCamera = true;
        }
...
void JamJar::Standard::WebGL2System::render(float deltaTime) {
    ...
    for (const auto &entityPair : this->m_entities) {
        ...
        auto camera = static_cast<JamJar::Standard::Camera *>(cameraEntity.Get(JamJar::Standard::Camera::CAMERA_KEY));
        auto transform =
            static_cast<JamJar::Standard::Transform *>(cameraEntity.Get(JamJar::Standard::Transform::TRANSFORM_KEY));
