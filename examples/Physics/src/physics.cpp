...
void Physics::OnStart() {
    auto bodyPtr = new JamJar::Standard::_2D::Box2DBody(JamJar::Polygon({-0.5, 0.5, 0.5, 0.5, 0.5, -0.5, -0.5, -0.5}));
    auto platform = new JamJar::Entity(messageBus);
    platform->Add(new JamJar::Standard::_2D::Transform(JamJar::Vector2D(0, -30), JamJar::Vector2D(150, 10)));
    platform->Add(new JamJar::Standard::_2D::Box2DBody(
    ...
    square->Add(new JamJar::Standard::_2D::Transform(JamJar::Vector2D(0, 30), JamJar::Vector2D(10, 10)));
    square->Add(new JamJar::Standard::_2D::Box2DBody(
        JamJar::Polygon({-0.5, 0.5, 0.5, 0.5, 0.5, -0.5, -0.5, -0.5}),
